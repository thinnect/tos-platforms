...
static void packetMode_RxPacketReceived(RAIL_Handle_t railHandle)
{
  ...
  if (rxPacket != NULL) {
    ...
    memcpy(rxPacket->dataPtr,
           packetInfo.firstPortionData,
           packetInfo.firstPortionBytes);
    if (packetInfo.firstPortionBytes != length) {
      memcpy(rxPacket->dataPtr + packetInfo.firstPortionBytes,
             packetInfo.lastPortionData, length - packetInfo.firstPortionBytes);
    }
...
static void fifoMode_RxPacketReceived(void)
{
  ...
  if (currentAppMode() == BER) {
    ...
    RAIL_GetRxPacketDetails(railHandle, RAIL_RX_PACKET_HANDLE_OLDEST,
...
void RAILCb_TxPacketSent(RAIL_Handle_t railHandle, bool isAck)
{
  ...
  if (isAck) {
    ...
    previousTxAckAppendedInfo.timeSent.timePosition = RAIL_PACKET_TIME_DEFAULT;
    ...
    previousTxAppendedInfo.timeSent.timePosition = RAIL_PACKET_TIME_DEFAULT;
    previousTxAppendedInfo.timeSent.totalPacketBytes = 0;
